...
export async function up(db: Kysely<Database>): Promise<void> {
  ...
  const desiredSchema: Record<string, TableDefinition> = {
    ...
    island: {
      ...
      labor_factory: { type: 'integer', config: (col) => col.defaultTo(0).notNull() },
      labor_mining: { type: 'integer', config: (col) => col.defaultTo(0).notNull() },
    ...
    turn_resource_history: {
      ...
      mining: { type: 'integer' },
      labor_factory: { type: 'integer' },
      labor_mining: { type: 'integer' },
      food_production: { type: 'integer' },
      food_consumption: { type: 'integer' },
    },
    plan: {
      from_uuid: { type: 'varchar(25)', config: (col) => col.notNull().references('user.uuid') },
